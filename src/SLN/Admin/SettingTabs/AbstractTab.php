...
abstract class SLN_Admin_SettingTabs_AbstractTab {
	...
	function __construct($slug, $label, $plugin) {
		...
		if ($_POST) {
            if (isset($_POST['reset']) && !empty($_POST[self::PAGE . $this->slug]) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST[self::PAGE . $this->slug])), self::PAGE . $this->slug)) {
                $this->processReset();
            } elseif (!empty($_POST[self::PAGE . $this->slug]) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST[self::PAGE . $this->slug])), self::PAGE . $this->slug)) {
				$this->process();
			} else {
				$this->showAlert(
					'error',
    ...
    public function processReset() {
        ...
        $this->showAlert(
            ...
            __('Update reset with success', 'salon-booking-system')
        );
    }
    ...
    private function resetSettings() {
        ...
        do_action('sln.settings.reset.after');
    }
