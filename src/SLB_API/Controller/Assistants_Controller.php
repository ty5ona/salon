...
class Assistants_Controller extends REST_Controller
{
    ...
    public function get_stats( $request )
    {
        ...
        foreach ($bookings_query->posts as $booking_post) {
            ...
            $booking_amount = $booking->getAmount();
                ...
                if ($attendant) {
                    ...
                    foreach ($service_assistants as $att) {
                        ...
                        if (isset($assistants_stats[$att_id])) {
                            // Count booking only once per assistant
                            if (!in_array($att_id, $processed_assistants)) {
                                $assistants_stats[$att_id]['bookings_count']++;
                                $processed_assistants[] = $att_id;
                            }
                            
                            // Add revenue (split evenly if multiple assistants on this service)
                            $revenue_share = $booking_amount / $num_assistants;
                            ...
                            if ($service_duration) {
                                ...
                                if ($minutes > 0) {
                                    $assistants_stats[$att_id]['total_hours_worked'] += $minutes / 60;
                                }
                            }
                        }
                    }
        ...
        foreach ($assistants_stats as $assistant_id => $stats) {
            ...
            // Simplified utilization calculation (would need working hours data for accurate calculation)
            // Assuming 8 hour days and 5 day weeks for simplicity
            $date_diff = (new \DateTime($start_date))->diff(new \DateTime($end_date));
            $weeks = max(1, floor($date_diff->days / 7));
            $available_hours = $weeks * 40; // 40 hours per week
